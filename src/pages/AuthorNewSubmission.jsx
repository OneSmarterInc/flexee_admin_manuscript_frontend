...
export default function AuthorNewSubmission() {
  ...
  function submit(e) {
    ...
    const draft = {
      ...
      manuscriptType: form.get('manuscriptType') || '',
      author: form.get('author') || '',
      coauthors: form.get('coauthors') || '',
      abstract: form.get('abstract') || '',
      keywords: form.get('keywords') || '',
      disclosure: form.get('disclosure') || '',
      filename: form.get('manuscript')?.name || '',
    }
    sessionStorage.setItem('authorDraft', JSON.stringify(draft))
    setBusy(true)
    window.setTimeout(() => go('/author/readiness'), 350)
  }
  ...
  return <PublicationShell>
    <div className="wrap author-flow-page">
      ...
      <div className="author-page-heading">
        ...
        <p className="publication-lede">Upload once. The same manuscript record will carry forward into readiness checks, venue comparison, and the destination you choose.</p>
      </div>
        ...
        <form className="author-workflow-form" onSubmit={submit}>
          <section className="author-form-section">
            <div className="author-form-section-head">
              <span>01</span>
              <div><h2>Manuscript</h2><p>The core file and information used for the readiness check.</p></div>
            </div>
            <div className="row">
            ...
            <div className="two">
              <div className="row">
                ...
                <select id="author-type" name="manuscriptType" required defaultValue="">
                  ...
                  <option>Practitioner article</option>
                  <option>Review article</option>
                  <option>Case study</option>
                  <option>Conference paper</option>
                  <option>Book manuscript</option>
                  <option>Other</option>
                </select>
              </div>
              <div className="row">
          ...
          <section className="author-form-section">
            <div className="author-form-section-head">
              <span>02</span>
              <div><h2>Authors and disclosure</h2><p>Keep reusable author information with the manuscript record.</p></div>
            </div>
            <div className="two">
              <div className="row">
          ...
          <div className="author-form-actions">
            <button className="author-secondary-button" type="button" onClick={() => go('/author')}>Save and exit</button>
            <button className="copper-button" type="submit" disabled={busy}>{busy ? 'Preparing preview…' : 'Run readiness check'}</button>
          </div>
        </form>

        <aside className="author-form-aside">
          <div className="author-panel author-guide-card">
            <p className="kicker">One manuscript record</p>
            <h2>Do this once.</h2>
            <p className="author-muted-copy">The target workflow reuses the same manuscript, author details, and disclosures when you compare outlets or transfer after a decision.</p>
          </div>
          <div className="author-mini-note">
            <b>Accepted in this preview</b>
